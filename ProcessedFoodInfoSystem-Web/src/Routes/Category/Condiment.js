...
const Condiment = (props) => {
  ...
  const [categoryName, setCategoryName] = useState('_조미료');
  ...
  useEffect(() => {
    ...
    console.log(sort);
    setCategoryName('조미료');
    if (sessionStorage.getItem('categoryName') === '_조미료' || sessionStorage.getItem('categoryName') === null) {
      console.log('대분류를 눌럿음');
      sessionStorage.setItem('categoryName', '_조미료');
  ...
  const getBigCategory = async (sort, pageNum, state) => {
    ...
    setCategoryName('_조미료');
    sessionStorage.setItem('categoryName', '_조미료');

    try {
      state === 'init' && setLoading(true);

      const { data } = await bigCategory.gotoCategory('조미료', pageNum, sort, 12);
