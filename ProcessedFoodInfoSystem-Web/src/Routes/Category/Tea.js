...
const Tea = (props) => {
  ...
  const [categoryName, setCategoryName] = useState('_음료/차');
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem('categoryName') === '_음료/차' || sessionStorage.getItem('categoryName') === null) {
      sessionStorage.setItem('categoryName', '_음료/차');
  ...
  const getBigCategory = async (sort, pageNum, state) => {
    ...
    setCategoryName('_음료/차');
    sessionStorage.setItem('categoryName', '_음료/차');

    try {
      state === 'init' && setLoading(true);

      const { data } = await bigCategory.gotoCategory('음료/차', pageNum, sort, 12);
